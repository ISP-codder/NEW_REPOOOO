...
async function notificationTemplate(data) {
    ...
    // 4. Основной текст (Добавлен отступ 1.25)
    children.push(
        new Paragraph({
    ...
    // 5. Сведения о факте (Добавлен отступ 1.25)
    children.push(
        new Paragraph({
            spacing: { before: 200 },
            indent: { firstLine: 708 },
    ...
    // 6. Законодательство (Добавлен отступ 1.25)
    children.push(
        new Paragraph({
            alignment: AlignmentType.JUSTIFY,
            spacing: { before: 200 },
    ...
    // 8. QR-код
    try {
        const qrPath = path.join(__dirname, '..', 'assets', 'images', 'static-qr.png')
    ...
    // 9. Контакты (Добавлен отступ 1.25)
    children.push(
        new Paragraph({
            spacing: { after: 100 },
            indent: { firstLine: 708 },
        ...
        new Paragraph({
            ...
            indent: { firstLine: 708 },
            children: [
                new TextRun({ text: 'E-mail: uk.ship.999@yandex.ru', size: 24, font: 'Times New Roman' }),
                new TextRun({ text: 'Телефон: +7 989 517-54-87', break: 1, size: 24, font: 'Times New Roman' }),
                new TextRun({ text: 'WhatsApp: +7 989 517-54-87', break: 1, size: 24, font: 'Times New Roman' })
            ]
        })
    )

    // 10. Рекомендация по WhatsApp (Добавлен отступ 1.25)
    children.push(
        new Paragraph({
            alignment: AlignmentType.JUSTIFY,
            spacing: { before: 200 },
    ...
    // 11. Предупреждение о суде (Добавлен отступ 1.25)
    children.push(
        new Paragraph({
            alignment: AlignmentType.JUSTIFY,
            spacing: { before: 200 },
    ...
    // 12. Подпись (Без изменений)
    children.push(
        new Paragraph({
            alignment: AlignmentType.LEFT,
            spacing: { before: 400 },
            children: [
                new TextRun({ text: 'С уважением,', size: 24, font: 'Times New Roman' }),
                new TextRun({ text: 'ООО «ЮК ШИП»', break: 1, size: 24, font: 'Times New Roman' })
            ]
        })
    )
